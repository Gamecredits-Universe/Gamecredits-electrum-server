...
if os.path.dirname(os.path.realpath(__file__)) == os.getcwd():
    imp.load_module('electrumgmcserver', *imp.find_module('src'))

from electrumgmcserver import storage, networks, utils
from electrumgmcserver.processor import Dispatcher, print_log
from electrumgmcserver.server_processor import ServerProcessor
from electrumgmcserver.blockchain_processor import BlockchainProcessor
from electrumgmcserver.stratum_tcp import TcpServer
from electrumgmcserver.stratum_http import HttpServer
...
def create_config(filename=None):
    ...
    config.set('server', 'banner_file', '/etc/electrum-gmc.banner')
    ...
    config.set('server', 'coin', 'gamecredits')
    config.set('server', 'logfile', '/var/log/electrum-gmc.log')
    config.set('server', 'donation_address', '')
    config.set('server', 'max_subscriptions', '10000')

    config.add_section('leveldb')
    config.set('leveldb', 'path', '/dev/shm/electrum-gmc_db')
    ...
    config.set('network', 'type', 'gamecredits_main')

    # try to find the config file in the default paths
    if not filename:
        for path in ('/etc/', ''):
            filename = path + 'electrum-gmc.conf'
