    ...
    def do_catch_up(self):
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("gamecreditsd is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def bitcoind(self, method, params=[]):
        ...
        while True:
            try:
                ...
                print_log("cannot reach gamecreditsd...")
                self.wait_on_bitcoind()
            else:
                r = loads(respdata)
                if r['error'] is not None:
                    if r['error'].get('code') == -28:
                        print_log("gamecreditsd still warming up...")
                        self.wait_on_bitcoind()
                        continue
		...
		error = e.args[0]
                if error["code"] == -26:
                    ...
                    if "non-mandatory-script-verify-flag" in message:
                        result = "Your client produced a transaction that is not accepted by the Gamecredits network any more. Please upgrade to Electrum 2.5.1 or newer\n"
                    else:
    ...
    def getfullblock(self, block_hash):
        ...
        while True:
            try:
                ...
                logger.error("gamecreditsd error (getfullblock)")
                self.wait_on_bitcoind()
                continue
            try:
                r = loads(respdata)
                rawtxdata = []
                for ir in r:
                    assert ir['error'] is None, "Error: make sure you run gamecreditsd with txindex=1; use -reindex if needed."
