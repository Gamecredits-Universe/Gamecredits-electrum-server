...
The most up-to date version of this document is available at:
    ...
    https://github.com/pooler/electrum-gmc-server/blob/master/HOWTO.md
...
document, we assume that user is called 'gamecredits'). We also assume the
gamecredits user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run gamecreditsd and keep a copy of the full blockchain, 
which is roughly 4 GB in July 2015. If you have less than 2 GB of RAM 
make sure you limit gamecreditsd to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running gamecreditsd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `gamecreditsd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser gamecredits --disabled-password
    $ sudo apt-get install git
    $ sudo su - gamecredits
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/gamecredits/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download gamecreditsd

We currently recommend gamecreditsd 0.10.2.2 stable.

If you prefer to compile gamecreditsd, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - gamecredits
    $ cd ~/src && git clone https://github.com/gamecredits-project/gamecredits.git -b master-0.10
    $ cd gamecredits
    $ ./autogen.sh
    $ ./configure --disable-wallet --without-miniupnpc
    $ make
    $ strip src/gamecreditsd src/gamecredits-cli src/gamecredits-tx
    $ cp -a src/gamecreditsd src/gamecredits-cli src/gamecredits-tx ~/bin

### Step 3. Configure and start gamecreditsd

In order to allow Electrum to "talk" to `gamecreditsd`, we need to set up an RPC
username and password for `gamecreditsd`. We will then start `gamecreditsd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.gamecredits
    $ $EDITOR ~/.gamecredits/gamecredits.conf

Write this in `gamecredits.conf`:
...
If you have an existing installation of gamecreditsd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ gamecreditsd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `gamecreditsd`:

    $ gamecreditsd

Allow some time to pass, so `gamecreditsd` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ gamecredits-cli getinfo

Before starting the electrum server your gamecreditsd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start gamecreditsd at boot
time, running as the 'gamecredits' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/pooler/electrum-gmc-server.git
    $ cd electrum-gmc-server
    $ sudo configure
...
and further instructions from the Electrum-GMC full archival server foundry at:
http://foundry.electrum-gmc.org/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-gmc.conf) when starting up. This
file includes the database setup, gamecreditsd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-gmc.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-gmc-server.
If you're using user gamecredits to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "gamecredits hard nofile 65536" >> /etc/security/limits.conf
     echo "gamecredits soft nofile 65536" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u gamecredits -i ulimit -n


Two more things for you to consider:

1. To increase security you may want to close gamecreditsd for incoming connections and connect outbound only

2. Consider restarting gamecreditsd (together with electrum-gmc-server) on a weekly basis to clear out unconfirmed
...
Note: If you want to run the server without installing it on your system, just run 'run_electrum_gmc_server" as the
unprivileged user.
...
If your system supports it, you may add electrum-gmc-server to the /etc/init.d directory. 
...
gamecredits to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-gmc and we'll try to congratulate you
on supporting the community by running an Electrum-GMC node.

If you're operating a public Electrum-GMC server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/electrum-gmc-server
It'll contain announcements about important updates to Electrum-GMC
